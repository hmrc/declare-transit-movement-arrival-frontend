...
class MrnDataRetrievalActionSpec extends FreeSpec with MustMatchers with GuiceOneAppPerSuite with ScalaFutures
  with MockitoSugar with ModelGenerators with OptionValues {

  val sessionRepository: SessionRepository = mock[SessionRepository]
  val userAnswersId: MovementReferenceNumber = arbitrary[MovementReferenceNumber].sample.value
  ...
  def harness(mrn: MovementReferenceNumber, f: OptionalDataRequest[AnyContent] => Unit): Unit = {
    ...
    lazy val actionProvider = app.injector.instanceOf[MrnDataRetrievalActionProviderImpl]
